...
use crate::intern::{Interner, InternedStr};
...
impl<'m, 't: 'm, 's: 't, V> ModuleWip<'m, 't, 's, V> {
    fn get_text(&mut self, n: Node<'t>) -> InternedStr<'m> {
        let s = self.src.as_bytes();
        self.interner.insert(n.utf8_text(s).unwrap()).unwrap()
    }
    ...
    fn process_literal_pattern(&mut self, n: Node<'t>) -> Result<Pattern<'m, &'m str>, ()> {
        match n.child(0) {
    ...
    fn process_eliminator_pattern(&mut self, n: Node<'t>) ->
    ...
    fn process_record_pattern(&mut self, n: Node<'t>) -> Result<Pattern<'m, &'m str>, ()> {
        panic!()
    }

    fn process_pattern(&mut self, n: Node<'t>) -> Result<Pattern<'m, &'m str>, ()> {
        match n.child(0) {
            Some(n) => match n.kind() {
                "literal_pattern" => self.process_literal_pattern(n),
                "(" => match n.next_sibling() {
                    Some(n) if n.kind() == "pattern" => {
                        self.process_pattern(n)
                    }
                    _ => has_bad_err!(n, self.errs),
                },
                "eliminator_pattern" => {
                    self.process_eliminator_pattern(n)
                }
        ...
        let bind = match n.child(i) {
            Some(n) if i == 2 || n.kind() == "binding" => match n.child(0) {
                Some(n) if n.kind() == "pattern" => {
                    ...
                    match p {
                        Ok(p) => match p.try_into_binding(&mut self.errs) {
                            Ok(bind) => bind,
                            _ => {
    ...
    fn process_top_value_signature(
        &mut self,
        n: Node<'t>,
    ) {
...
pub enum BadCstError<'a> {
    InternalError(Reason, Node<'a>),
}

impl<'a> BadCstError<'a> {
    fn is_unexpected_child(n: Node<'a>) -> BadCstError<'a> {
        BadCstError::InternalError(Reason::IsUnexpectedChild, n)
    }
    fn has_unexpected_children(n: Node<'a>) -> BadCstError<'a> {
        BadCstError::InternalError(Reason::HasUnexpectedChildren, n)
    }
}
                ...
                "definition" => {
                    for n in non_comment_children(&n) {
                        match n.kind() {
                            ...
                            "top_type_signature" => {}
                            "top_type_definition" => {}
                            _ => m.errs.push(BadCstError::is_unexpected_child(n)),
                        }
                    }
                }
                _ => {
